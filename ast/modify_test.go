...
func TestModify(t *testing.T) {
	...
	tests := []struct {
		...
		{&FunctionLiteral{Parameters: []*Identifier{}, Body: &BlockStatement{Statements: []Statement{&ExpressionStatement{Expression: one()}}}},
		&FunctionLiteral{Parameters: []*Identifier{}, Body: &BlockStatement{Statements: []Statement{&ExpressionStatement{Expression: two()}}}}},
		{&ArrayLiteral{Elements: []Expression{one(), one(), one()}},
	...
	for _, tt := range tests {
		...
		if !equal {
			t.Errorf("not equal. got=%q, want=%q", modified.String(), tt.expected.String())
		}
	}
	...
	for key, val := range hashLiteral.Pairs {
		...
		if key.Value != 2 {
			t.Errorf("kvalue is not %d, got=%d", 2, key.Value)
		}
		val, _ := val.(*IntegerLiteral)
		if val.Value != 2 {
			t.Errorf("vvalue is not %d, got=%d", 2, val.Value)
		}
	}
}